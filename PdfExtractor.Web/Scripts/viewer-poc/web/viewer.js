        ...
        (this, function (exports, pdfjsLib) {
            ...
            var TextLayerBuilder = (function TextLayerBuilderClosure() {
                function TextLayerBuilder(options) {
                    ...
                    this._overlays_activated = false;
                    this._rowMap;
                    this._sizeGroups;
                    this._verticalOrdering;
                    this._spatialHashPowerX = 6;
                    this._spatialHashPowerY = 2;
                    this._spatialHash = new SpatialHash(this._spatialHashPowerX, this._spatialHashPowerY);
                    this._bindMouse();
                }
                ...
                TextLayerBuilder.prototype = {
                    ...
                    _normalizedFromPx: function TextLayerBuilder_Lee_normalizedFromPx (pos, places) {
                        ...
                        if (len > 0 && pos.substring(len - 2) === "px") {
                            pos = pos.substring(0, len - 2);
                        }
                        return this._normalizeDecimal(pos, places);
                    },

                    _getNormalizedPosition: function TextLayerBuilder_Lee_getNormalizedPosition(elem) {
                        var self = this;
                        var places = 2;
                        var haveStyle = !!elem && !!elem.style;
                        function get(prop) {
                            return haveStyle && elem.style[prop] ? self._normalizedFromPx (elem.style[prop], places) : "undefined";
                        }
                        function get(prop) {
                            return haveStyle && elem.style[prop] ? self._normalizedFromPx (elem.style[prop], places) : "undefined";
                        }
                        var $elem = $(elem);
                        function getWidth() {
                            //return self._normalizeDecimal($elem.attr ('data-canvas-width'), places);
                            return self._normalizeDecimal($elem.width(), places);
                        }
                        function getHeight() {
                    ...
                    _getNormalizedFontInfo: function TextLayerBuilder_Lee_getNormalizedFontInfo(elem) {
                        ...
                        var places = 3;
                        var fontSize = haveStyle ? this._normalizedFromPx(style.fontSize, places) : "";
                        var fontFamily = haveStyle ? style.fontFamily : "";
                        var fontWeight = haveStyle ? style.fontWeight : "";
                        return {
                            fontSize: fontSize,
                    ...
                    _buildSizeGroups: function TextLayerBuilder_Lee_buildSizeGroups () {
                        ...
                        $.each(this.textDivs, function(idx, div) {
                            var fi = self._getNormalizedFontInfo(div);
                            var fk = JSON.stringify(fi);
                            var group = self._sizeGroups [fk];
                            if (!group) {
                                group = [];
                                self._sizeGroups [fk] = group;
                            }
                            group.push(div);
                        });
                    },
                    ...
                    _getElemsInSameSizeGroupAs: function TextLayerBuilder_Lee_getElemsInSameSizeGroupAs (elem) {
                        ...
                        var fk = JSON.stringify(fi);
                        var group = this._sizeGroups [fk].slice (0);
                        if (group) {
                            var idx = group.indexOf (de);
                            if (idx !== -1) {
                                group.splice (idx, 1);
                            }
                        }
                    ...
                    _populateSpatialHash: function TextLayerBuilder_Lee_populateSpatialHash () {
                        ...
                        var self = this;
                        $.each(this.textDivs, function(idx, div) {
                            var obj = self._elemToObj (div);
                            self._spatialHash.insert (obj);
                        });
                        var bucketCount = this._spatialHash.getNumBuckets ();
                    ...
                    // _buildVerticalOrdering: function TextLayerBuilder_Lee_buildVerticalOrdering () {
                        ...
                        // });
                        // this._verticalOrdering.sort (function (one, two) {
                            // var posOne = self._getNormalizedPosition(one);
                            // var posTwo = self._getNormalizedPosition(two);
                            // return posOne.top - posTwo.top;
                        // });
                    // },
                    ...
                    _buildRowMap: function TextLayerBuilder_Lee_buildRowMap() {
                        this._rowMap = {};
                        var self = this;
                        $.each(this.textDivs, function(idx, div) {
                            var pos = self._getNormalizedPosition(div);
                            var y = pos.top;// - parentPos.top;
                            var row = self._rowMap [y];
                            if (!row) {
                                row = [];
                                self._rowMap [y] = row;
                            }
                    ...
                    _getElemsInSameRowAs: function TextLayerBuilder_Lee_getElemsInSameRowAs (elem) {
                        ...
                        var y = pos.top;// - parentPos.top;
                        var row = this._rowMap [y].slice (0);
                        if (row) {
                            var idx = row.indexOf (de);
                            if (idx !== -1) {
                                row.splice (idx, 1);
                            }
                        }
                    ...
                    _activateOverlays: function TextLayerBuilder_Lee_activateOverlays() {
                        ...
                        this._overlays_activated = true;
                        var self = this;

                        this._buildSizeGroups();
                        this._buildRowMap();
                        ...
                        $(this.textLayerDiv).on({
                            mouseenter: function (e) {
                                ...
                                if (alt) {
                                    ...
                                    if (row) {
                                        shiftLog ("has row-children...");
                                        $.each (row, function (idx, el) {
                                            $(el).addClass ('same-row');
                                            shiftLog(idx + ") " + self._dumpElem(el, true));
                                        });
                                    }
                                }
                                ...
                                // if (group) {
                                    ...
                                    // $.each (group, function (idx, el) {
                                        // $(el).addClass ('same-group');
                                        // //console.log(idx + ") " + self._dumpElem(el, true));
                                    // });
                                // }
    ...
    (function (root, factory) { {
        ...
        (this, function (exports, pdfjsLib) {
            ...
            function scrollIntoView(element, spot, skipOverflowHiddenElements) {
                ...
                parent = document.body;  // Lee - cope with different DOM expectations to allow scrolling to page
                if (!parent) {
    ...
    (function (root, factory) { {
        ...
        (this, function (exports, uiUtilsLib, firefoxComLib, downloadManagerLib,
            ...
            document.addEventListener('pagerendered', function (e) {
                ...
                var canvases = $.map(PDFViewerApplication.pdfViewer._pages,
                    function(p) {
                        return p.canvas;
                    });

                function getWpdPlotDataFromMemento(wpdPlotMemento) {

                    var plotData = wpdPlotMemento.plotData,
                        axes = plotData.axes,
                ...
                function buildPlotlyGraphFromWpdData(data) {
                    if (!data || !data.datasets) {
                        return null;
                    }
                    var colours = ['red', 'green', 'blue', 'orange'];
                    var cc = colours.length;

                    var pyData = $.map(data.datasets,
                        function(ds, i) {
                            var trace = {
                                name: ds.name,
                                mode: 'lines+markers',
                                ...
                                x: $.map(ds.points, function(p, i) {
                                    return p.dx;
                                }),
                                y: $.map(ds.points, function (p, i) {
                                    return p.dy;
                                })
                            };
                            return trace;
                    ...
                    var div = $('<div></div>')[0];
                    Plotly.newPlot(div, pyData, layout);

                    return div;
                }

                function showDataForSelection(sel) {
                    var wpdPlotMemento = sel.dataStore.getData().data;
                    var data = getWpdPlotDataFromMemento(wpdPlotMemento);
                    var graph = buildPlotlyGraphFromWpdData(data);

                    var modal = new jBox('Modal', {
                        content: $(graph),
                        title: "Graph data extracted",
                        closeOnEsc: true,
                        closeButton: 'box',
                ...
                function transplantedDoStuff(canvas) {
                    var ctx = canvas.getContext('2d');
                    ctx.save();
                    ...
                    function setStrokeRGBColor(r, g, b) {
                        var color = makeCssRgb(r, g, b);
                    ...
                    if (wpd.utils.getDevOptions().grid) {
                        drawGrid([0, 0, 255]);
                    }

                    ctx.lineWidth = 2;
                    for (var i = 0; i < canvas._imageInfo.length; i++) {

                        var ii = canvas._imageInfo[i];
                        var w = ii.image.width * ii.derived.scale.x;
                        var h = ii.image.height * ii.derived.scale.y;
                 ...
                 // canvas._imageInfo = []; <-- don't discard the imageInfo
                }

                window.selectionRectangles.init(
                    canvases,
                    {
                        $growthContainer: $('#viewerContainer'),
                        selectionHoveredHandler: function (sel) {
                            if (sel && sel.canvas) {
                            }
                        },
                        selectionUnhoveredHandler: function(sel) {
                            if (sel && sel.canvas) {
                            }
                        },
                        selectionNumberClickedHandler: function(sel) {
                            if (sel && sel.canvas) {
                                ...
                                if (!zoomedCloneImgSrc) {
                                    ...
                                    sel.dataStore.putData({ zoomedCloneImgSrc: zoomedCloneImgSrc });
                                }
                                var wpdPlotMemento = si.data;

                                var wpdConfig = window.PDFViewerApplication_wpdConfig;
                        ...
                        dataIconClickedHandler: function (sel) {
                            if (sel && sel.canvas) {
                                showDataForSelection(sel);
                            }
                        }
                    });

                transplantedDoStuff(canvases [canvases.length - 1]);
            ...
            var pdfSandboxContainer = document.getElementsByClassName('pdfSandboxContainer')[0];
            pdfSandboxContainer.addEventListener('DOMMouseScroll', handleMouseWheel);
            pdfSandboxContainer.addEventListener('mousewheel', handleMouseWheel);
            ...
            window.addEventListener('keydown', function keydown(evt) {
                if ($(document.body).hasClass('wpd-visible')) {
                    return;
                }
                if (OverlayManager.active) {
                    return;
...
function webViewerLoad(wpdConfig) {
    var config = getViewerConfiguration();
    window.PDFViewerApplication_wpdConfig = wpdConfig;
